...
from .models import (
    ...
    Photo, GalleryPhoto, PhotoGroup, GalleryGroup, ComplexGalleryGroup, HeatingGalleryGroup, VerificationGalleryGroup, EmergencyPhotoGroup, AuditGalleryGroup, PreparationGalleryGroup, VentilationPhotoGroup, CommercialProposal, HeatingPhotoGroup, ComplexPhotoGroup
)
...
class HeatingPhotoGroupAdmin(PhotoAdmin):
    """Админ для группы Фото Комплексное обслуживание ИТП"""
...
class ComplexPhotoGroupAdmin(PhotoAdmin):
    """Админ для группы Фото команды Комплексное обслуживание"""
    
    def get_queryset(self, request):
        qs = super(PhotoAdmin, self).get_queryset(request)
        return qs.filter(photo_type__startswith='complex_team')
...
admin.site.register(ComplexPhotoGroup, ComplexPhotoGroupAdmin)
...
ComplexPhotoGroup._meta.verbose_name = "Фото команды Комплексное обслуживание инженерных систем"
ComplexPhotoGroup._meta.verbose_name_plural = "Фото команды Комплексное обслуживание инженерных систем"